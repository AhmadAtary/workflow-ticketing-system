        ...
        <TabsList className="border-b border-border bg-transparent rounded-none w-full justify-start p-0 h-auto gap-0">
          {[
            ...
            <TabsTrigger key={tab.value} value={tab.value}
              className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:text-primary pb-3 px-4 text-sm font-medium flex items-center gap-2">
          ...
          <Card className="border-border max-w-2xl">
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Input value={general.companyName} onChange={e => setGeneral(p => ({ ...p, companyName: e.target.value }))} className="mt-1 max-w-sm" />
              </div>
              <div className="flex items-center justify-between py-3 border-b border-border">
                ...
                <Switch checked={general.allowRegistration} onCheckedChange={v => setGeneral(p => ({ ...p, allowRegistration: v }))} />
              </div>
              <div className="flex items-center justify-between py-3">
                ...
                <Switch checked={general.requireEmailVerification} onCheckedChange={v => setGeneral(p => ({ ...p, requireEmailVerification: v }))} />
              </div>
          ...
          <Card className="border-border max-w-2xl">
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between py-2">
                ...
                <Switch checked={mail.emailEnabled} onCheckedChange={v => setMail(p => ({ ...p, emailEnabled: v }))} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label>SMTP Host</Label>
                  <Input value={mail.emailHost} onChange={e => setMail(p => ({ ...p, emailHost: e.target.value }))} placeholder="smtp.gmail.com" className="mt-1" disabled={!mail.emailEnabled} />
                </div>
                <div>
                  <Label>SMTP Port</Label>
                  <Input type="number" value={mail.emailPort} onChange={e => setMail(p => ({ ...p, emailPort: Number(e.target.value) }))} placeholder="587" className="mt-1" disabled={!mail.emailEnabled} />
                </div>
                <div>
                  <Label>From Email</Label>
                  <Input type="email" value={mail.emailFrom} onChange={e => setMail(p => ({ ...p, emailFrom: e.target.value }))} placeholder="noreply@company.com" className="mt-1" disabled={!mail.emailEnabled} />
                </div>
                <div>
                  <Label>Username</Label>
                  <Input value={mail.emailUser} onChange={e => setMail(p => ({ ...p, emailUser: e.target.value }))} placeholder="SMTP username" className="mt-1" disabled={!mail.emailEnabled} />
                </div>
                <div>
                  <Label>Password</Label>
                  <Input type="password" value={mail.emailPassword} onChange={e => setMail(p => ({ ...p, emailPassword: e.target.value }))} placeholder="SMTP password" className="mt-1" disabled={!mail.emailEnabled} />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-1">
                  <input type="color" value={branding.primaryColor} onChange={e => setBranding(p => ({ ...p, primaryColor: e.target.value }))}
                    className="h-9 w-16 rounded border border-border cursor-pointer p-0.5" />
                  <Input value={branding.primaryColor} onChange={e => setBranding(p => ({ ...p, primaryColor: e.target.value }))} className="max-w-[140px] font-mono" />
                </div>
              </div>
              <div>
                <Label>Logo URL</Label>
                <Input value={branding.logoUrl} onChange={e => setBranding(p => ({ ...p, logoUrl: e.target.value }))} placeholder="https://..." className="mt-1 max-w-sm" />
              </div>
