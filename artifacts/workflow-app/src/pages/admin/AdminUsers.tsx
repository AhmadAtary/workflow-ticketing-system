...
export default function AdminUsers() {
  ...
  const [form, setForm] = useState({ name: "", email: "", role: "user", teamId: "" });

  const filtered = users.filter(u => {
    if (search && !u.name.toLowerCase().includes(search.toLowerCase()) && !u.email.toLowerCase().includes(search.toLowerCase())) return false;
    if (filterRole !== "all" && u.role !== filterRole) return false;
    if (filterTeam !== "all" && u.teamId !== filterTeam) return false;
    return true;
  });
  ...
  const handleCreate = () => {
    ...
    setShowCreate(false);
    setForm({ name: "", email: "", role: "user", teamId: "" });
  };

  const handleEdit = () => {
    if (!form.name || !form.email) { toast({ title: "Name and email are required", variant: "destructive" }); return; }
    updateUser(showEdit.id, { name: form.name, email: form.email, role: form.role as any, teamId: form.teamId || undefined, teamName: teams.find(t => t.id === form.teamId)?.name });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => { setForm({ name: "", email: "", role: "user", teamId: "" }); setShowCreate(true); }}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="John Smith" className="mt-1" />
                </div>
                <div>
                  <Label>Email *</Label>
                  <Input type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} placeholder="john@company.com" className="mt-1" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
      ...
      <Dialog open={!!deleteConfirm} onOpenChange={() => setDeleteConfirm(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={() => { deleteUser(deleteConfirm.id); toast({ title: "User deleted" }); setDeleteConfirm(null); }}>Delete</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
